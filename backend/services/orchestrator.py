...
async def expand_query(request: QueryRequest) -> QuestionExpansion:
    ...
    # Construir contexto desde la memoria compartida
    memory_context = shared_memory.build_conversation_context(
        request.company_id, request.conversation_id or ""
    )
    full_context = memory_context or request.conversation_context or "Sin contexto previo."
